...
theme_questions (){
	while [ "x$theme" == "x" ]
	do
		theme=$(whiptail --title "Theme" --backtitle "$back_title" --nocancel --radiolist "Select Theme" 10 50 3 "ISPC-Clean" "(default)" ON  "ISPC-Material" "(new)" OFF "Other" "" OFF 3>&1 1>&2 2>&3)
	done
...
function_install_ISPC_Material() {
	...
	# Get Theme
	cd /tmp
	wget --no-check-certificate -O master.zip https://codeload.github.com/kOld/ISPConfig_Clean-3.0.5/zip/master
	unzip master.zip
	cd ISPConfig_Clean-3.0.5-master
	cp -R interface/* /usr/local/ispconfig/interface/
